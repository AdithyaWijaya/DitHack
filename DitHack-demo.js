...
(function () {
  ...
  async function validateLicense(licenseCode) {
    try {
      const response = await fetch(
        `https://api-dithack.up.railway.app/check/${licenseCode}`,
      );
      ...
      if (!response.ok) {
        return { valid: false, message: "Server error, hubungi admin!" };
      }
  ...
  function createFrame() {
    ...
    title.textContent = "DitHack!";
    title.style.cssText = `font-size: ${isMobile ? "14px" : "16px"}; flex: 1; font-weight: bold;`;
  ...
  function createToggleButton() {
    ...
    img.src = "https://dithack.vercel.app/frontend/src/ico/icodh.png";
    img.alt = "DitHack";
    img.style.cssText = `
  ...
  async function fetchAnswers(pin) {
    try {
      const response = await fetch(
        `https://api-dithack.up.railway.app/quizizz?pin=${encodeURIComponent(pin)}`,
        {
          headers: {
  ...
  async function fetchKahootAnswers(link) {
    try {
      const response = await fetch(
        `https://api-dithack.up.railway.app/kahoot?link=${encodeURIComponent(link)}`,
        {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("auth"),
          },
