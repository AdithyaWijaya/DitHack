...
from backend.app import schemas, dependencies
from backend.app.models import CacheQuizizz, CacheKahoot
from backend.app.dependencies import get_db, get_license_status, _validate_and_bind_device, _get_cheatnetwork_account_for_license, _log_usage, _extract_device_info
from backend.app.config import CACHE_DURATION

router = APIRouter(prefix="", tags=["api"])

@router.get("/check/{token}", response_model=schemas.LicenseResponse)
def check_token(
    token: str,
    request: Request,
    ...
    lic = dependencies.get_license(db, token)
    if not lic:
